...
def main_update_arthro(id):
    ...
    def get_init():
        con = pymysql.connect(host = '150.140.186.221',port = 3306, user='db20_up1059338',passwd='up1059338', database='project_db20_up1059338')              
        cur = con.cursor()
        cur.execute("SELECT a.Τίτλος, a.Αρχική_Σελίδα, a.Τελική_Σελίδα, p.Κωδικός_Περιοδικού, p.Όνομα, t.Αριθμός_Τεύχους, s1.Κωδικός_Συντάκτη, s2.Όνομα, s2.Επώνυμο, s1.Κωδικός_Ιδρύματος, i.Όνομα, a.Κωδικός_Άρθρου FROM Συντάσσει s1 JOIN ΑΡΘΡΟ a ON a.Κωδικός_Άρθρου = s1.Κωδικός_Άρθρου JOIN ΣΥΝΤΑΚΤΗΣ s2 ON s1.Κωδικός_Συντάκτη = s2.Κωδικός_Συντάκτη JOIN ΙΔΡΥΜΑ i ON s1.Κωδικός_Ιδρύματος = i.Κωδικός_Ιδρύματος LEFT JOIN ΤΕΥΧΟΣ_ΠΕΡΙΟΔΙΚΟΥ t ON a.Κωδικός_Τεύχους = t.Κωδικός_Τεύχους LEFT JOIN ΠΕΡΙΟΔΙΚΟ p ON t.Κωδικός_Περιοδικού = p.Κωδικός_Περιοδικού WHERE a.Κωδικός_Άρθρου = " + id_arthrou )
        r = cur.fetchall()
        cur.close()
        con.close()
        return r
    ...
    def submit():
        ...
        cur = con.cursor()
        con.commit()
        sql_command = "UPDATE ΑΡΘΡΟ SET Τίτλος = %s, Αρχική_Σελίδα = %s, Τελική_Σελίδα = %s WHERE Κωδικός_Άρθρου = %s"
        values = (title.get(),int(start_page.get()),int(end_page.get()), id_arthrou)
        ...
        con = pymysql.connect(host = '150.140.186.221',port = 3306, user='db20_up1059338',passwd='up1059338', database='project_db20_up1059338')              
        cur = con.cursor()
        sql_command = "DELETE FROM Συντάσσει WHERE Κωδικός_Άρθρου = %s"
        cur.execute(sql_command, (id_arthrou,))
        con.commit()
    ...
    def init_entries_arthro():
        r = get_init()
        title.insert(0, str(r[0][0]))
        start_page.insert(0, r[0][1])
        end_page.insert(0, r[0][2])
        periodiko.set(str(r[0][3])+" "+str(r[0][4]))
        vol_teuxous.set(str(r[0][5]))

        global id_arthrou
        id_arthrou = r[0][11]
        sintaktis[0].set(str(r[0][6])+" "+str(r[0][7])+" "+str(r[0][8]))
        idrima[0].set(str(r[0][9])+" "+str(r[0][10]))
        sintaktis_selected('<<ComboboxSelected>>', 0)
        idrima_selected('<<ComboboxSelected>>', 0)
        global i
        for p in range(1,len(r)):
            multiple_sintakti()
            #--------------------na prosthesv na patietai koumpi--------------
            sintaktis[i].set(str(r[i][6])+" "+str(r[i][7])+" "+str(r[i][8]))
            idrima[i].set(str(r[0][9])+" "+str(r[0][10]))
            sintaktis_selected('<<ComboboxSelected>>', i)
            idrima_selected('<<ComboboxSelected>>', i)

    root = Tk()
    root.title('Επεξεργασία Άρθρου στη Βάση')
