...
def sintaktis_selected(event, x):
    global id_sintakti
    id_sintakti.append(sintaktis[x].get()[0])

def idrima_selected(event, x):
    global id_idrima
    id_idrima.append(idrima[x].get()[0])
...
def get_init():
    con = pymysql.connect(host = '150.140.186.221',port = 3306, user='db20_up1059338',passwd='up1059338', database='project_db20_up1059338')              
    cur = con.cursor()
    cur.execute("SELECT a.Τίτλος, a.Αρχική_Σελίδα, a.Τελική_Σελίδα, p.Κωδικός_Περιοδικού, p.Όνομα, t.Αριθμός_Τεύχους FROM ΑΡΘΡΟ a LEFT JOIN Συντάσσει s1 ON a.Κωδικός_Άρθρου = s1.Κωδικός_Άρθρου LEFT JOIN ΣΥΝΤΑΚΤΗΣ s2 ON s1.Κωδικός_Συντάκτη = s2.Κωδικός_Συντάκτη LEFT JOIN ΙΔΡΥΜΑ i ON s1.Κωδικός_Ιδρύματος = i.Κωδικός_Ιδρύματος LEFT JOIN ΤΕΥΧΟΣ_ΠΕΡΙΟΔΙΚΟΥ t ON a.Κωδικός_Τεύχους = t.Κωδικός_Τεύχους LEFT JOIN ΠΕΡΙΟΔΙΚΟ p ON t.Κωδικός_Περιοδικού = p.Κωδικός_Περιοδικού WHERE a.Κωδικός_Άρθρου = " + id_arthrou )
    r = cur.fetchall()
    cur.close()
    con.close()
    return r
...
def getid():
    con = pymysql.connect(host = '150.140.186.221',port = 3306, user='db20_up1059338',passwd='up1059338', database='project_db20_up1059338')              
    cur = con.cursor()
    cur.execute("SELECT Κωδικός_Άρθρου, Τίτλος FROM ΑΡΘΡΟ WHERE Αρχική_Σελίδα=%s AND Κωδικός_Τεύχους=%s" % (int(start_page.get()), id_teuxos))
    r = [x[0] for x in cur.fetchall()][0]
    cur.close()
    con.close()
    return r
...
def submit():
    ...
    for index in range(len(sintaktis)):
        ...
        print(index,id_sintakti[index], getid(), id_idrima[index])
